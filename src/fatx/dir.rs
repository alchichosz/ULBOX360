...
use crate::fatx::datetime::DateTime;
use crate::fatx::error::Error;
use crate::fatx::fat::{ClusterId, FatEntry};
use crate::fatx::fs::{FatxFs, FatxFsHandle};
use crate::fatx::path::normalize_virtual_path;
...
impl DirectoryEntryIterator {
    ...
    pub(crate) fn next(&mut self, fs: &mut FatxFs) -> Option<Result<DirectoryEntry, Error>> {
        ...
        match crate::fatx::read_struct::<_, DirectoryEntry>(&mut fs.device_handle) {
            Err(err) => {
                self.finished = true;
                Some(Err(err.into()))
            }
